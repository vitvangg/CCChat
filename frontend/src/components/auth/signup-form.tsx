...
export function SignupForm({
  ...
  async function onSubmit(data: SignupFormValues) {
    ...
    try {
      ...
      navigate("/sign-in"); // chỉ redirect khi thành công
    } catch (error) {
      // không redirect, toast đã hiện trong store
      console.error(error);
    }
  }
          ...
          <form className="p-6 md:p-8" onSubmit={handleSubmit(onSubmit)}>
            <FieldGroup>
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <h1 className="text-base font-bold">Tạo tài khoản</h1>
                <p className="text-muted-foreground text-sm text-balance">
                  Chào mừng bạn đến với CCChat! Hãy tạo tài khoản để bắt đầu!
                </p>
              </div>
              <Field>
              ...
              <FieldDescription className="text-center">
                Đã có tài khoản ? <a href="/sign-in" className="text-chart">Đăng nhập</a>
              </FieldDescription>
            </FieldGroup>
          </form>
